...
{  
    ...
    if (bFileName)
    {
        const string kf_file =  "kf_" + string(argv[argc-1]) + "_EuRoC.txt";
        const string f_file =  "f_" + string(argv[argc-1]) + "_EuRoC.txt";
        SLAM.SaveTrajectoryEuRoC(f_file);
        SLAM.SaveKeyFrameTrajectoryEuRoC(kf_file);
    }
    else
    {
        SLAM.SaveTrajectoryEuRoC("CamTraj_EuRoC.txt");
        SLAM.SaveKeyFrameTrajectoryEuRoC("KFTraj_EuRoC.txt");
        SLAM.SaveTrajectoryTUM("CamTraj_TUM.txt");
        SLAM.SaveKeyFrameTrajectoryTUM("KFTraj_TUM.txt");
    }
