...
void Atlas::PostLoad()
{
    ...
    mspMaps.clear();
    unsigned long int numKF = 0, numMP = 0, numML = 0;
    ...
    for(Map* pMi : mvpBackupMaps)
    {
        ...
        pMi->PostLoad(mpKeyFrameDB, mpORBVocabulary, mpLineVocabulary, mpKeyFrameId, mpCams);
        mpAllKeyFrameId.insert(mpKeyFrameId.begin(), mpKeyFrameId.end());
        numKF += pMi->GetAllKeyFrames().size();
        numMP += pMi->GetAllMapPoints().size();
        numML += pMi->GetAllMapLines().size(); //LINE
    }
...
void Atlas::SetLineVocabulary(LineVocabulary* pLineVoc)
{
...
LineVocabulary* Atlas::GetLineVocabulary()
{
    return mpLineVocabulary;
}
