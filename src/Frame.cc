...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpORBvocabulary(frame.mpORBvocabulary), mpLinevocabulary(frame.mpLinevocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
     ...
     mvbOutlier(frame.mvbOutlier), mvbOutlier_Line(frame.mvbOutlier_Line), mImuCalib(frame.mImuCalib), mnCloseMPs(frame.mnCloseMPs),
     ...
     inv_width(frame.inv_width), inv_height(frame.inv_width),
     ...
     mpCamera(pCamera) ,mpCamera2(nullptr), mTimeStereoMatch(0), mTimeORB_Ext(0)
{
    ...
    N = mvKeys.size();
    //N_l = mvKeys_Line.size(); //new

    if(mvKeys.empty())
        return;
    if(mvKeys_Line.empty())
        return;
    ...
    mvpMapLines = vector<MapLine*>(N_l,static_cast<MapLine*>(NULL)); //new
    mvbOutlier = vector<bool>(N,false);
    mvbOutlier_Line = vector<bool>(N_l,false); //new
...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, 
    ...
    GeometricCamera* pCamera,Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc), mpLinevocabulary(voc_l),
     mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)), mpLineextractorLeft(Lineextractor),
     mTimeStamp(timeStamp), mK(K.clone()),mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth),
     mImuCalib(ImuCalib), mpImuPreintegrated(NULL), mpPrevFrame(pPrevF), mpImuPreintegratedFrame(NULL), mpReferenceKF(static_cast<KeyFrame*>(NULL)), mbImuPreintegrated(false),
     mpCamera(pCamera),mpCamera2(nullptr), mTimeStereoMatch(0), mTimeORB_Ext(0)
{
...
Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor* extractor, Lineextractor* Lineextractor,
    ORBVocabulary* voc, LineVocabulary* voc_l, GeometricCamera* pCamera, 
...
void Frame::ExtractORBandLine(int flag, const cv::Mat &im, const int x0, const int x1)
{
    vector<int> vLapping = {x0,x1};
    if(flag==0)
    {
