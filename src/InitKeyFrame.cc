...
namespace ORB_SLAM3
{
    InitKeyFrame::InitKeyFrame(SystemSetting &SS):pORBvocabulary(SS.pORBvocabulary),pLinevocabulary(SS.pLinevocabulary)
    {
        fx = SS.fx;
        fy = SS.fy;
        cx = SS.cx;
    ...
    void InitKeyFrame::UndistortKeyLines()
    {
        if(DistCoef.at<float>(0)==0.0)
        {
            vKeysUn_Line = vKeys_Line;
            return;
        }

        N_l = vKeys_Line.size();   // update N_l
        ...
        for(int i=0; i<N_l; i++)
        {
            mat_s.at<float>(i,0)=vKeys_Line[i].startPointX;
            mat_s.at<float>(i,1)=vKeys_Line[i].startPointY;
            mat_e.at<float>(i,0)=vKeys_Line[i].endPointX;
            mat_e.at<float>(i,1)=vKeys_Line[i].endPointY;
        }

        // Undistort points
        mat_s=mat_s.reshape(2);
        mat_e=mat_e.reshape(2);
        cv::undistortPoints(mat_s,mat_s,K,DistCoef,cv::Mat(),K);
        cv::undistortPoints(mat_e,mat_e,K,DistCoef,cv::Mat(),K);
        mat_s=mat_s.reshape(1);
        mat_e=mat_e.reshape(1);

        // Fill undistorted keypoint vector
        vKeysUn_Line.resize(N_l);
        for(int i=0; i<N_l; i++)
        {
            vKeysUn_Line[i].startPointX=mat_s.at<float>(i,0);
            vKeysUn_Line[i].startPointY=mat_s.at<float>(i,1);
            vKeysUn_Line[i].endPointX=mat_e.at<float>(i,0);
            vKeysUn_Line[i].endPointY=mat_e.at<float>(i,1);
        }
    }
