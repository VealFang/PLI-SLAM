...
LoopClosing::LoopClosing(Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, LineVocabulary* pVoc_l, const bool bFixScale):
                    ...
                    if(numOptMatches >= nSim3Inliers)
                    {
                        ...
                        if(numProjOptMatches >= nProjOptMatches)
                        {
                            ...
                            for(MapPoint* pMPi : vpMatchedMP)
                            {
                                ...
                                size_t index = pMPi->GetIndexInKeyFrame(pKFi);
                                if(index >= 0)
                                {
