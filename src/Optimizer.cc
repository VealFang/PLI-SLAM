    ...
    for(size_t i=0; i<vpMPs.size(); i++)
    {
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad())
            {
                ...
                else //if(pKFi->mvuRight[leftIndex] >= 0) // stereo observation
                {
                    //kpUn = pKFi->mvKeysUn[leftIndex];
                    Eigen::Matrix<double,3,1> obs;
                    obs << kpUn.pt.x, kpUn.pt.y;
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                ...
                else if(pKFi->mvuRight[mit->second]>=0)// Stereo observation (with rectified images)
                {
                    const cv::KeyPoint &kpUn = pKFi->mvKeysUn[mit->second];
                    Eigen::Matrix<double,3,1> obs;
                    //const float kp_ur = pKFi->mvuRight[mit->first];
                    obs << kpUn.pt.x, kpUn.pt.y;
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                ...
                else if(pKFi->mvuRight[mit->second]>=0)// Stereo observation
                {
                    const cv::KeyPoint &kpUn = pKFi->mvKeysUn[mit->second];
                    Eigen::Matrix<double,3,1> obs;
                            ...
                            const bool bFixScale, Eigen::Matrix<double,7,7> &mAcumHessian, const bool bAllPoints)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        pKFm = vpMatches1KF[i];
        const int i2 = pMP2->GetIndexInKeyFrame(pKFm);
        if(i2 < 0)
...
void Optimizer::MergeBundleAdjustmentVisual(KeyFrame* pCurrentKF, vector<KeyFrame*> vpWeldingKFs, vector<KeyFrame*> vpFixedKFs, bool *pbStopFlag)
{
    ...
    for(unsigned int i=0; i < vpMPs.size(); ++i)
    {
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            KeyFrame* pKF = mit->first;
            if(spKeyFrameBA.find(pKF) == spKeyFrameBA.end() || pKF->isBad() || pKF->mnId>maxKFid || pKF->mnBALocalForKF != pCurrentKF->mnId || !pKF->GetMapPoint(mit->second))
                continue;
        ...
        if(dist > 1.0)
        {
            if(bShowImages)
            {
                ...
                for(int i=0; i<vpMonoMPsOpt.size(); ++i)
                {
                    ...
                    int index = vpMonoMPsOpt[i]->GetIndexInKeyFrame(pKFi);
                    if(index < 0)
                    {
                 ...
                 for(int i=0; i<vpStereoMPsOpt.size(); ++i)
                 {
                     ...
                     int index = vpStereoMPsOpt[i]->GetIndexInKeyFrame(pKFi);
                     if(index < 0)
                     {
