...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, LineVocabulary* pVoc_l, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, const string &_nameSeq):
    ...
    mbOnlyTracking(false), mbMapUpdated(false), mbVO(false), mpORBVocabulary(pVoc), mpLineVocabulary(pVoc_l), 
    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr)
{
    ...
    if(sensor==System::MONOCULAR || sensor==System::IMU_MONOCULAR)
        ...
        mpIniLineextractor = new Lineextractor(2*lsd_nfeatures, min_line_length, lsd_refine,
                                    lsd_scale, lsd_sigma_scale, lsd_quant, lsd_ang_th, lsd_log_eps, lsd_density_th, lsd_n_bins, false);
    ...
    else if(sCameraName == "KannalaBrandt8")
    {
        ...
        if(mSensor==System::STEREO || mSensor==System::IMU_STEREO){
            ...
            if(!b_miss_params)
            {
                ...
                mpCamera2 = new KannalaBrandt8(vCamCalib2); //fisheye
...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    else if(mSensor == System::STEREO && mpCamera2) //stereo fisheye
        mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,
    ...
    else if(mSensor == System::IMU_STEREO && !mpCamera2)
    {
        ...
        //cout<<"[DEBUG] frame constructed..."<<endl;
    }
        
    else if(mSensor == System::IMU_STEREO && mpCamera2) //stereo IMU fisheye
        mCurrentFrame = Frame(mImGray,imGrayRight,timestamp,mpORBextractorLeft,mpORBextractorRight,
        mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,mpCamera2,mTlr,&mLastFrame,*mpImuCalib);
...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    std::chrono::steady_clock::time_point t0 = std::chrono::steady_clock::now();
    mCurrentFrame = Frame(mImGray,imDepth,timestamp,mpORBextractorLeft,mpLineextractorLeft,
    mpORBVocabulary,mpLineVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
...
cv::Mat Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)
            mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpIniLineextractor,
            mpORBVocabulary,mpLineVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
        else
            mCurrentFrame = Frame(mImGray,timestamp,mpORBextractorLeft,mpLineextractorLeft,
            mpORBVocabulary,mpLineVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
    }
    else if(mSensor == System::IMU_MONOCULAR)
    {
        ...
        {
            mCurrentFrame = Frame(mImGray,timestamp,mpIniORBextractor,mpIniLineextractor,
            mpORBVocabulary,mpLineVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
        }
        else
            mCurrentFrame = Frame(mImGray,timestamp,mpORBextractorLeft,mpLineextractorLeft,
            mpORBVocabulary,mpLineVocabulary,mpCamera,mDistCoef,mbf,mThDepth,&mLastFrame,*mpImuCalib);
...
void Tracking::Track()
{
    ...
    //cout<<"[DEBUG] Start Tracking..."<<endl;
    if (bStepByStep)
    {
        while(!mbStep)
            usleep(500);
    ...
    if ((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO) && !mbCreatedMap)
    {
        ...
        //cout<<"[DEBUG] IMU Preintegrated..."<<endl;
    }
    mbCreatedMap = false;
        ...
        std::chrono::steady_clock::time_point timeStartPosePredict = std::chrono::steady_clock::now();
            ...
            if(mState==OK)
            {
                ...
                if((mVelocity.empty() && !pCurrentMap->isImuInitialized()) || mCurrentFrame.mnId<mnLastRelocFrameId+2)
                {
                    ...
                    if(mSensor==System::IMU_MONOCULAR || mSensor==System::MONOCULAR)
                    {
                        bOK = TrackReferenceKeyFrame();
                    }
                    else
                    {
                        bOK = TrackReferenceKeyFrameWithLine(); //changed
                    }
                }
                else
                {
                    ...
                    {
                        bOK = TrackWithMotionModelWithLine(); //changed
                        if(!bOK)
...
void Tracking::StereoInitialization()
{
    if(mCurrentFrame.N>500)
    {
        ...
        // Create MapPoints & MapLines and asscoiate to KeyFrame
        ...
        if(!mpCamera2){
            ...
            for(int i=0; i<mCurrentFrame.N_l; ++i)
            {
                if(mCurrentFrame.mvDisparity_l[i].first>0 && mCurrentFrame.mvDisparity_l[i].second>0)
                {
        ...
        Verbose::PrintMess("New Map created with " + to_string(mpAtlas->MapPointsInMap()) + " points & " + to_string(mpAtlas->MapLinesInMap()) + " lines", Verbose::VERBOSITY_QUIET);
        ...
        mvpLocalMapLines=mpAtlas->GetAllMapLines(); //new
        mpReferenceKF = pKFini;
        mCurrentFrame.mpReferenceKF = pKFini;

        mpAtlas->SetReferenceMapPoints(mvpLocalMapPoints);
        mpAtlas->SetReferenceMapLines(mvpLocalMapLines); //new
...
{
    //cout<<"[DEBUG]Monocular Initialization..."<<endl;
    if(!mpInitializer)
    {
        // Set Reference Frame
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            //fill(mvIniLineMatches.begin(),mvIniLineMatches.end(),-1); //line
    ...
    else
    {
        ...
        //Linematcher lmatcher(0.9,true);
        //int lineMatches = lmatcher.SearchDouble(mInitialFrame,mCurrentFrame,mvIniLineMatches);
        ...
        //cout<<"[DEBUG]nmatches = "<< nmatches <<", lineMatches = "<< lineMatches <<endl;

        cv::Mat Rcw; // Current Camera Rotation
        cv::Mat tcw; // Current Camera Translation
        vector<bool> vbTriangulated; // Triangulated Correspondences (mvIniMatches)
        //vector<bool> mvbLineTriangulated;   //new: 匹配的线特征是否能够三角化
        ...
        //new: 通过H或者F进行单目初始化，得到两帧之间相对运动，初始化MapPoints
        ...
        if(mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn,mCurrentFrame.mvKeysUn,mvIniMatches,Rcw,tcw,mvIniP3D,vbTriangulated))
        {
            Verbose::PrintMess("Initializing MapPoints...", Verbose::VERBOSITY_QUIET); //for debug
            for(size_t i=0, iend=mvIniMatches.size(); i<iend;i++)
            {
                if(mvIniMatches[i]>=0 && !vbTriangulated[i])
                {
            ...
            //cout<<"[DEBUG]mappoints initialized..."<<endl;
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    //cout<<"[DEBUG]KeyFrame added..."<<endl;
    ...
    /*cout<<"[DEBUG]mvIniLineMatches = "<< mvIniLineMatches.size() <<endl;
    for(size_t i=0; i<mvIniLineMatches.size();i++)
    {
        //if(mvIniLineMatches[i]<0)
            //continue;
        //cout<<"mvIniLineMatches[i]: "<<mvIniLineMatches[i]<<endl;

        //Create MapLine.
        //if(mCurrentFrame.mvDisparity_l[i].first>0 && mCurrentFrame.mvDisparity_l[i].second>0)
        //{
            cout<<"[DEBUG]Disparity_l[i].first = "<< mCurrentFrame.mvDisparity_l[i].first << ", second = " << mCurrentFrame.mvDisparity_l[i].second <<endl;

        cout<<"[DEBUG]start Creatting MapLine..."<<endl;
        Eigen::Vector3d sp3D = mCurrentFrame.backProjection(
                    mCurrentFrame.mvKeysUn_Line[i].startPointX,
                    mCurrentFrame.mvKeysUn_Line[i].startPointY,
                    mCurrentFrame.mvDisparity_l[i].first);
        ...
        //mCurrentFrame.mvbOutlier_Line[mvIniLineMatches[i]] = false;
        cout<<"[DEBUG]line_3..."<<endl;

        pKFini->AddMapLine(pML,i);
        pKFcur->AddMapLine(pML,i);
        //pKFcur->AddMapLine(pML,mvIniLineMatches[i]);
        cout<<"[DEBUG]line_4..."<<endl;
        
        //Add to Map
        mpAtlas->AddMapLine(pML);
        //}
    }
    cout<<"[DEBUG]MapLines created..."<<endl;*/

    // Update Connections
    pKFini->UpdateConnections();
    pKFcur->UpdateConnections();
    //cout<<"[DEBUG]connections updated..."<<endl;
...
{
    // step1:创建关键帧，即用于初始化的前两帧al MapPoin
    KeyFrame* pKFini = new KeyFrame(mInitialFrame,mpAtlas->GetCurrentMap(),mpKeyFrameDB);
    KeyFrame* pKFcur = new KeyFrame(mCurrentFrame,mpAtlas->GetCurrentMap(),mpKeyFrameDB);

    Verbose::PrintMess("KeyFrame has been created...", Verbose::VERBOSITY_QUIET); //for debug
    ...
    cout<<"[DEBUG]MapPoints created..."<<endl;
    ...
    for(size_t i=0; i<mvIniLineMatches.size(); i++)
    {
        ...
        MapLine* pML = new MapLine(worldPos, pKFcur, mpAtlas->GetCurrentMap());
    ...
    cout << "CreateInitialMapMonoWithLine: this Map created with " << mpAtlas->GetCurrentMap()->MapPointsInMap() << " points, and "<< mpAtlas->GetCurrentMap()->MapLinesInMap() << " lines." << endl;
    Optimizer::GlobalBundleAdjustemnt(mpAtlas->GetCurrentMap(), 20); 
    ...
    mpLocalMapper->InsertKeyFrame(pKFini);
    mpLocalMapper->InsertKeyFrame(pKFcur);
    cout << "CreateInitialMapMonoWithLine: KeyFrame has been inserted..." << endl; //for debug
    ...
    cout << "Reference MapPoints & MapLines have been set..." << endl; //for debug
...
bool Tracking::TrackReferenceKeyFrameWithLine()
{
    ...
    if(mCurrentFrame.n_inliers_pt<15 && mCurrentFrame.n_inliers_ls<10) {
        cout<<"[Debug] In TrackReferenceKeyFrameWithLine(), mCurrentFrame's n_inliers_pt<15&&n_inliers_ls<10 -> return false"<<endl;
...
void Tracking::UpdateLastFrame()
{
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        if(bCreateNew)
        {
            Eigen::Vector3d sp3D = mLastFrame.backProjection(
                        mLastFrame.mvKeysUn_Line[i].startPointX,
                        mLastFrame.mvKeysUn_Line[i].startPointY,
                        mLastFrame.mvDisparity_l[i].first);
            Eigen::Vector3d ep3D = mLastFrame.backProjection(
                        mLastFrame.mvKeysUn_Line[i].endPointX,
                        mLastFrame.mvKeysUn_Line[i].endPointY,
                        mLastFrame.mvDisparity_l[i].second);
...
bool Tracking::TrackLocalMap()
{
    ...
    UpdateLocalMap();
    SearchLocalPointsAndLines(); //changed

    mCurrentFrame.SetprevInformation(mLastFrame.mTcw, mLastFrame.DT_cov, mLastFrame.err_norm); //new
...
void Tracking::CreateNewKeyFrame()
{
    ...
    if(mSensor!=System::MONOCULAR && mSensor != System::IMU_MONOCULAR) // TODO check if incluide imu_stereo
    {
        ...
        // If there are less than 100 close lines we create the 100 closest.
        vDepthIdx.clear();
        vDepthIdx.reserve(mCurrentFrame.N_l);
        for(int i=0; i<mCurrentFrame.N_l; i++)
        {
            float sz = mCurrentFrame.mbf/mCurrentFrame.mvDisparity_l[i].first;
            float ez = mCurrentFrame.mbf/mCurrentFrame.mvDisparity_l[i].second;
...
void Tracking::SearchLocalPointsAndLines()
{
    ...
    if(nToMatch>0)
    {
        match(mvpLocalMapLines_InFrustum,mCurrentFrame,Config::minRatio12L(), matches_12);
    }
    ...
    mCurrentFrame.n_inliers_pt = 0;
    for(auto it=mCurrentFrame.mvpMapPoints.begin(),itend=mCurrentFrame.mvpMapPoints.end(); it!=itend; ++it)
        if(*it) ++mCurrentFrame.n_inliers_pt;
    mCurrentFrame.n_inliers_ls = 0;
...
//new
std::vector<MapLine*> Tracking::GetLocalMapMLS()
{
